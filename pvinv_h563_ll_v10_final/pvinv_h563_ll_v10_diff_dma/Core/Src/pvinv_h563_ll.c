...
volatile uint16_t g_pvinv_adc1_raw[PVINV_ADC1_DIFF_CH_NUM];
volatile uint16_t g_pvinv_adc2_raw[PVINV_ADC2_DIFF_CH_NUM];
 ...
 */
static uint16_t g_adc1_latched[PVINV_ADC1_DIFF_CH_NUM];
static uint16_t g_adc2_latched[PVINV_ADC2_DIFF_CH_NUM];
...
{
    for (uint32_t i = 0u; i < PVINV_ADC1_DIFF_CH_NUM; i++)
    {
...
{
    for (uint32_t i = 0u; i < PVINV_ADC2_DIFF_CH_NUM; i++)
    {
...
static uint16_t adc_raw_from_source_snapshot(uint8_t src,
                                             const uint16_t adc1_snapshot[PVINV_ADC1_DIFF_CH_NUM],
                                             const uint16_t adc2_snapshot[PVINV_ADC2_DIFF_CH_NUM])
{
    switch (src)
    {
        case PVINV_ADC_SRC_ADC1_IN10:
            return adc1_snapshot[PVINV_ADC1_IDX_IN10];
        case PVINV_ADC_SRC_ADC1_IN12:
            return adc1_snapshot[PVINV_ADC1_IDX_IN12];
        case PVINV_ADC_SRC_ADC2_IN1:
            return adc2_snapshot[PVINV_ADC2_IDX_IN1];
        case PVINV_ADC_SRC_ADC2_IN18:
            return adc2_snapshot[PVINV_ADC2_IDX_IN18];
...
static uint8_t adc_source_is_valid(uint8_t src)
{
    ...
    {
        case PVINV_ADC_SRC_ADC1_IN10:
            return (PVINV_ADC1_IDX_IN10 < PVINV_ADC1_DIFF_CH_NUM) ? 1u : 0u;
        case PVINV_ADC_SRC_ADC1_IN12:
            return (PVINV_ADC1_IDX_IN12 < PVINV_ADC1_DIFF_CH_NUM) ? 1u : 0u;
        case PVINV_ADC_SRC_ADC2_IN1:
            return (PVINV_ADC2_IDX_IN1 < PVINV_ADC2_DIFF_CH_NUM) ? 1u : 0u;
        case PVINV_ADC_SRC_ADC2_IN18:
            return (PVINV_ADC2_IDX_IN18 < PVINV_ADC2_DIFF_CH_NUM) ? 1u : 0u;
        case PVINV_ADC_SRC_NONE:
            return 1u;
        default:
            return 0u;
...
{
    float vdiff = adc_diff_raw_to_voltage(raw);
    return PVINV_UD_SIGN * (vdiff * PVINV_UD_GAIN + PVINV_UD_OFFSET);
}

static inline float adc_to_id(uint16_t raw)
{
    float vdiff = adc_diff_raw_to_voltage(raw);
    float i = vdiff / (PVINV_ID_SHUNT_R_OHM * PVINV_ID_TOTAL_GAIN);
...
{
    float vdiff = adc_diff_raw_to_voltage(raw);
    return PVINV_UREF_SIGN * (vdiff * PVINV_UREF_GAIN + PVINV_UREF_OFFSET);
}

static inline float adc_to_ifb(uint16_t raw)
{
    float vdiff = adc_diff_raw_to_voltage(raw);
    float i = vdiff / (PVINV_IFB_SHUNT_R_OHM * PVINV_IFB_TOTAL_GAIN);
...
static inline float adc_to_uo(uint16_t raw)
{
    ...
    float vdiff = adc_diff_raw_to_voltage(raw);
    return PVINV_UO_SIGN * (vdiff * PVINV_UO_GAIN + PVINV_UO_OFFSET);
}
...
void PVINV_LL_ControlISR(void)
{
    ...
    uint16_t adc1_snapshot[PVINV_ADC1_DIFF_CH_NUM];
    uint16_t adc2_snapshot[PVINV_ADC2_DIFF_CH_NUM];
    for (uint32_t i = 0u; i < PVINV_ADC1_DIFF_CH_NUM; i++)
    {
    ...
    g_inv.code_ud = adc_diff_raw_to_code(raw_ud);
    g_inv.code_id = adc_diff_raw_to_code(raw_id);
    g_inv.code_uref = adc_diff_raw_to_code(raw_uref);
    g_inv.code_ifb = adc_diff_raw_to_code(raw_ifb);
    g_inv.code_uo = (PVINV_SRC_UO == PVINV_ADC_SRC_NONE) ? 0 : adc_diff_raw_to_code(raw_uo);

    g_inv.vdiff_ud = adc_diff_raw_to_voltage(raw_ud);
    g_inv.vdiff_id = adc_diff_raw_to_voltage(raw_id);
    g_inv.vdiff_uref = adc_diff_raw_to_voltage(raw_uref);
    g_inv.vdiff_ifb = adc_diff_raw_to_voltage(raw_ifb);
    g_inv.vdiff_uo = (PVINV_SRC_UO == PVINV_ADC_SRC_NONE) ? 0.0f : adc_diff_raw_to_voltage(raw_uo);
