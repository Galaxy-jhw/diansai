...
static float MPPT_Update(MPPT_IncCond_t *m, float v, float i, float *g_out)
{
    ...
    else
    {
        ...
        {
            /* MPP 左侧，Ud 偏低，需要 Ud 升高；本系统减小 Iamp。 */
            m->iamp_target -= step;
            m->probe_dir = -1;
        }
        else if (g < -PVINV_MPPT_G_EPS)
        {
            /* MPP 右侧，Ud 偏高，需要 Ud 降低；本系统增大 Iamp。 */
            m->iamp_target += step;
            m->probe_dir = 1;
        }
        else
...
static uint8_t IsFaultState(PVINV_State_t s)
{
    return (s == PVINV_STATE_FAULT_UNDERVOLTAGE) ||
           (s == PVINV_STATE_FAULT_OVERVOLTAGE) ||
           (s == PVINV_STATE_FAULT_OVERCURRENT) ||
           (s == PVINV_STATE_FAULT_REF_LOST) ||
           (s == PVINV_STATE_FAULT_PWM_BREAK);
}
...
static void FaultRecover_Task(void)
{
    ...
    switch (g_inv.state)
    {
        ...
        case PVINV_STATE_FAULT_PWM_BREAK:
            /* Break 必须人工确认后调用 PVINV_LL_ClearPwmBreakFaultAfterCheck()。 */
            recover_ok = 0u;
            break;
...
void PVINV_LL_ClearPwmBreakFaultAfterCheck(void)
{
    ...
    /* 只清标志，不立即打开 PWM；重新回到 WAIT_REF 走软启动。 */
    PWM_ClearBreakFlags();
    g_inv.fault_cnt = 0u;
    g_inv.iamp_target = 0.0f;
    g_inv.iamp_cmd = 0.0f;
...
void PVINV_LL_ControlISR(void)
{
    ...
    /* 1. ADC raw -> 物理量 */
    g_inv.ud   = adc_to_ud(g_pvinv_adc_raw[PVINV_ADC_IDX_UD]);
    g_inv.id   = adc_to_id(g_pvinv_adc_raw[PVINV_ADC_IDX_ID]);
    g_inv.uref = adc_to_uref(g_pvinv_adc_raw[PVINV_ADC_IDX_UREF]);
    g_inv.ifb  = adc_to_ifb(g_pvinv_adc_raw[PVINV_ADC_IDX_IFB]);
    g_inv.uo   = adc_to_uo(g_pvinv_adc_raw[PVINV_ADC_IDX_UO]);
