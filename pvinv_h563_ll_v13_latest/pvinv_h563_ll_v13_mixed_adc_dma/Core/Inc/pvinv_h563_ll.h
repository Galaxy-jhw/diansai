...
#define PVINV_ADC1_CH_NUM               2u
#define PVINV_ADC2_CH_NUM               2u
 ...
 * 所以V13保留“通道->数组下标”的宏，并新增源映射查重、ADC1/ADC2成对完成保护、调试原始码输出、源类型自适应换算和DMA错误Hook。只要CubeMX Rank顺序变了，改下面四个宏即可，
 * 控制算法主体不需要改。
 *
 * 默认推荐Rank：
 ...
 *   ADC2 Rank2 = ADC2_IN18 Single-ended -> g_pvinv_adc2_raw[1]
 *
 * 注意：ADC2_IN18用于信号发生器参考正弦uREF时，必须保证信号发生器输出
 * 已经被偏置/限幅到STM32 ADC允许范围内，不能直接输入负电压。
 */
#ifndef PVINV_ADC1_IDX_IN10_DIFF
#define PVINV_ADC1_IDX_IN10_DIFF                  0u
#endif
#ifndef PVINV_ADC1_IDX_IN12_DIFF
#define PVINV_ADC1_IDX_IN12_DIFF                  1u
#endif
#ifndef PVINV_ADC2_IDX_IN1_DIFF
#define PVINV_ADC2_IDX_IN1_DIFF                   0u
#endif
#ifndef PVINV_ADC2_IDX_IN18_SINGLE
#define PVINV_ADC2_IDX_IN18_SINGLE                  1u
#endif

extern volatile uint16_t g_pvinv_adc1_raw[PVINV_ADC1_CH_NUM];
extern volatile uint16_t g_pvinv_adc2_raw[PVINV_ADC2_CH_NUM];
...
#define PVINV_ADC_SRC_ADC2_IN1_DIFF               3u
#define PVINV_ADC_SRC_ADC2_IN18_SINGLE            4u

#define PVINV_ADC_SRC_TYPE_NONE                   0u
#define PVINV_ADC_SRC_TYPE_DIFF                   1u
#define PVINV_ADC_SRC_TYPE_SINGLE                 2u

/*
 * 你已明确要求：参考正弦uREF来自信号发生器，采用单端ADC测量。
 * 保持该宏为1时，若PVINV_SRC_UREF不是ADC2_IN18_SINGLE，编译会报错，避免误配。
 */
#ifndef PVINV_REQUIRE_UREF_SINGLE
#define PVINV_REQUIRE_UREF_SINGLE                 1u
#endif
...
/*
 * ADC1_IN10、ADC1_IN12、ADC2_IN1按差分ADC解释；ADC2_IN18按单端ADC解释。
 * 差分ADC必须上板验证：INP>INN时解释结果为正，INP<INN时为负，INP=INN时接近0。
 */
...
#error "PVINV_ADC_DIFF_RES_BITS must be in [2,16]. Set it to CubeMX ADC differential resolution."
#endif
#if (PVINV_ADC_SINGLE_RES_BITS < 2u) || (PVINV_ADC_SINGLE_RES_BITS > 16u)
#error "PVINV_ADC_SINGLE_RES_BITS must be in [2,16]. Set it to CubeMX ADC single-ended resolution."
#endif
...
#define PVINV_ADC_SINGLE_CODE_MASK           ((uint32_t)((1u << PVINV_ADC_SINGLE_RES_BITS) - 1u))
#define PVINV_ADC_SINGLE_FULL_SCALE          ((float)((1u << PVINV_ADC_SINGLE_RES_BITS) - 1u))

/* Ud/uo：若使用差分采样，physical = vdiff * GAIN + OFFSET */
#define PVINV_UD_GAIN                        20.0f
#define PVINV_UD_OFFSET                      0.0f
#define PVINV_UD_SIGN                        1.0f
#define PVINV_UD_ZERO_VOLT                   0.0f

#define PVINV_UO_GAIN                        20.0f
#define PVINV_UO_OFFSET                      0.0f
#define PVINV_UO_SIGN                        1.0f
#define PVINV_UO_ZERO_VOLT                   0.0f

/*
 * uREF由信号发生器直接给出，推荐接ADC2_IN18 Single-ended。
 * STM32 ADC不能输入负电压，所以信号发生器应输出带直流偏置的波形，例如：
 *   1.65V偏置，幅值不超过约1.5V，使ADC引脚电压始终在0~3.3V范围内。
 * 软件中先把ADC单端码换成电压，再减去零点PVINV_UREF_ZERO_VOLT，得到以0为中心的参考信号。
 */
...
#define PVINV_ID_ZERO_VOLT                   0.0f
...
#define PVINV_IFB_ZERO_VOLT                  0.0f
...
#if (PVINV_ADC1_IDX_IN10_DIFF >= PVINV_ADC1_CH_NUM)
#error "PVINV_ADC1_IDX_IN10_DIFF out of g_pvinv_adc1_raw[] range."
#endif
#if (PVINV_ADC1_IDX_IN12_DIFF >= PVINV_ADC1_CH_NUM)
#error "PVINV_ADC1_IDX_IN12_DIFF out of g_pvinv_adc1_raw[] range."
#endif
#if (PVINV_ADC2_IDX_IN1_DIFF >= PVINV_ADC2_CH_NUM)
#error "PVINV_ADC2_IDX_IN1_DIFF out of g_pvinv_adc2_raw[] range."
#endif
#if (PVINV_ADC2_IDX_IN18_SINGLE >= PVINV_ADC2_CH_NUM)
#error "PVINV_ADC2_IDX_IN18_SINGLE out of g_pvinv_adc2_raw[] range."
#endif

#if (PVINV_ADC1_IDX_IN10_DIFF == PVINV_ADC1_IDX_IN12_DIFF)
#error "ADC1 IN10 and IN12 cannot map to the same DMA index."
#endif
#if (PVINV_ADC2_IDX_IN1_DIFF == PVINV_ADC2_IDX_IN18_SINGLE)
#error "ADC2 IN1 and IN18 cannot map to the same DMA index."
#endif
#if PVINV_REQUIRE_UREF_SINGLE
#if (PVINV_SRC_UREF != PVINV_ADC_SRC_ADC2_IN18_SINGLE)
#error "uREF is required to be single-ended from ADC2_IN18. Set PVINV_REQUIRE_UREF_SINGLE=0 only if you intentionally change hardware."
#endif
#endif
...
typedef struct
{
    ...
    uint32_t adc1_dma_error_count;
    uint32_t adc2_dma_error_count;
...
/* 手动喂数：用于无DMA时调试。参数顺序固定为ADC1_IN10_DIFF、ADC1_IN12_DIFF、ADC2_IN1_DIFF、ADC2_IN18_SINGLE。 */
